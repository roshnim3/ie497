module i_cache (
    ...
    // linebuffer control signals (linebuffer moved to fetch stage)
    input   logic   [26:0]  linebuf_tagout,
    input   logic   [255:0] linebuf_dout,
    output  logic   [26:0]  linebuf_tagin,
    output  logic   [255:0] linebuf_din,
    output  logic           linebuf_wr_en
);
    enum logic [2:0] {
        IDLE,
        HIT,
    ...
    // Line buffer signals (linebuffer moved to fetch stage)
    logic linebuf_hit;

    assign linebuf_hit = (linebuf_tagout == ufp_addr[31:5]);
    assign linebuf_tagin = ufp_addr[31:5];
    assign linebuf_din = (state == HIT) ? mem_data : dfp_rdata;
    assign linebuf_wr_en = |hit_way && (state == HIT) || (dfp_resp && (state == ALLOCATE));
    ...
    always_comb begin
        ufp_rdata = 32'b0;
        ufp_resp = 1'b0;
        dfp_addr = (state == ALLOCATE) ? mem_addr & 32'hFFFFFFE0 : (state == WRITEBACK) ? dfp_addr_reg : '0;  
        dfp_read = (state == ALLOCATE && !dfp_resp) ? 1'b1 : 1'b0;
        ...
        case (state)
            IDLE: begin
                // send a request to all arrays to read a line
                if(linebuf_hit) begin
                    next_state = IDLE;
                    ufp_resp = 1'b1;
                    ufp_rdata = {mask[3] ? linebuf_dout[32 * ufp_addr[4:2] + 24 +: 8] : 8'b0,
                                    mask[2] ? linebuf_dout[32 * ufp_addr[4:2] + 16 +: 8] : 8'b0,
                                    mask[1] ? linebuf_dout[32 * ufp_addr[4:2] + 8  +: 8] : 8'b0,
                                    mask[0] ? linebuf_dout[32 * ufp_addr[4:2]      +: 8] : 8'b0};
                end else begin
            ...
            HIT: begin
                ...
                if (|hit_way) begin // if there is a cache hit
                    ...
                    if (|(dirty_dout & evict_next)) begin
                        ...
                        tag_addr = (evict_next[0] ? tag_dout[0] : '0) |   
            ...
            ALLOCATE: begin
                next_state = (dfp_resp == 1'b1) ? IDLE : ALLOCATE;                
                if(dfp_resp == 1'b1) begin // turn off read once response is received
            ...
            WRITEBACK: begin
                next_state = (dfp_resp == 1'b1) ? ALLOCATE : WRITEBACK;            
                end
            default: begin
                next_state = IDLE;
            end
