...
//   - The seq slot is written last so the firmware never sees a
//     half-written packet by polling slot 3 for changes.
//   - After the 5-cycle burst, idles for (interval - 5) cycles before the
//     next packet, simulating inter-arrival time.
//   - `seq_write_pulse` fires for one cycle when slot 3 is written, so
//     the testbench can timestamp packet publication for the latency
//     histogram.
...
(
    ...
    logic [31:0] phase;
    logic [31:0] seq_num;        // next seq to publish (starts at 1)
    logic [2:0]  rom_idx;
