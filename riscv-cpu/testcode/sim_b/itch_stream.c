...
void main(void) {
    ...
    for (uint32_t i = 0; i < N_PACKETS; i++) {
        // Poll the FIFO empty flag (slot 4) until the parser publishes
        // a packet. Returns 1 if empty, 0 if at least one packet pending.
        while (FETCH_TRADE(4)) {
            // spin
        }

        // Read the head packet's fields.
        ...
        // Pop the head packet (advance the FIFO tail). Slot 5 returns 1
        // when the pop succeeds; we don't need the value here.
        (void)FETCH_TRADE(5);
